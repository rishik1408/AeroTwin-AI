...
def main():
    ...
    for i in range(args.healthy):
        simulate_mission(f"healthy_{i}", args.outdir, fault_type=None)
    ...
    for f_type in fault_types:
        ...
        for i in range(args.faulty):
            ...
            simulate_mission(f"fault_{f_type}_{i}", args.outdir, fault_type=f_type, fault_onset=onset, fault_duration=duration, severity=severity)
